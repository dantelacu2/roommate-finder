...
// In this file we execute our KNN matching algorithm:
// 1. calculate the weighted Euclidean distance between two profiles
// 2. find the k nearest neighbors based on the weighted distances
// 3. find the k nearest neighbors indexed off a particular profile
...
function weightedEuclideanDistance(profile1, profile2, weights) {
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    if (
      typeof profile1[key] === "number" &&
      typeof profile2[key] === "number" &&
      weights[key] &&
      ...
      if (profile1[key + "_importance"]) {
        if (key === "move_in_date") {
          sum +=
            weights[key] * Math.pow(monthDiff(profile1[key], profile2[key]), 2);
        } else {
          sum +=
            weights[key] *
            (profile1[key + "_importance"] / 3) *
            Math.pow(profile1[key] - profile2[key], 2);
        }
      } else {
